...
public class ServerMain {
	...
	public class ClientHandler implements Runnable	{
		...
		public void run() {
                        String message;
			String[] data;
                        String connect = "Conectare";
                        String disconnect = "Deconectare";
                        String chat = "Chat";
			try {
				while ((message = reader.readLine()) != null) {
					System.out.println("Primit: " + message);
					data = message.split("L");
                                        for (String token:data) {                                           
                                        System.out.println(token);                                       
                                        }                                       
                                        if (data[2].equals(connect)) {
                                                tellEveryone((data[0] + "L" + data[1] + "L" + chat));
                                                userAdd(data[0]);
					} else if (data[2].equals(disconnect)) {
                                            tellEveryone((data[0] + "Lhas disconnected." + "L" + chat));
                                            userRemove(data[0]);
	...
	public void userAdd (String data) {
                String message;
                String add = "L LConnect", done = "ServerL LDone";
